...
def _message_post_helper(
        res_model, res_id, message, token='',
        _hash=False, pid=False, nosubscribe=True, **kw):
    ...
    if token or (_hash and pid):
        ...
        if _check_special_access(res_model, res_id, token=token, _hash=_hash, pid=pid):
            ...
            raise Forbidden()
    elif not kw.get('document_access'):  # early check on access to avoid useless computation
    ...
    return record.sudo().with_context(mail_create_nosubscribe=nosubscribe).message_post(**message_post_args)


class TaxsuretyPortalChatter(PortalChatter):
    ...
    def portal_chatter_post(self, res_model, res_id, message,
        ...
        Inherited:
            To add document access for portal users
        """
        ...
        self._portal_post_check_attachments(attachment_ids or [],
        ...
        #     Add chatter document access
        post_values = {
            'res_model': res_model,
            'res_id': res_id,
            'document_access': True if kw.get('document_access') else False,
            'message': message,
            'send_after_commit': False,
            'attachment_ids': False,  # will be added afterward
        }
        post_values.update((fname, kw.get(fname)) for fname in
                           self._portal_post_filter_params())
